...
현재는 준비물 설명·설계 단계다. 이 문서 작성으로 설치·계정 생성·결제·배포가 완료된 것은 아니다.
...
| PDC_Diary_Backend | Gradle Wrapper·Spring MVC/Validation/JDBC·MariaDB 드라이버·Flyway·JUnit/Testcontainers·Dockerfile |
...
- 현재 로그인 없이 모든 요구 기능을 제공한다. AWS·GitHub의 관리 계정은 서비스 이용자 로그인과 별개다.
...
| 준비물 | 목적·조건 |
| --- | --- |
| AWS 계정·결제수단 | Lightsail 서버·백업 운영. 계정 MFA, 제한된 운영 권한과 비용 알림 설정 |
...
공개 웹은 `https://plandosee.app`, 공통 API는 `https://plandosee.app/api/v1`이다. DB 3306·내부 웹/API 포트는 외부에 공개하지 않는다. 도메인 구매는 사용자 진술로 확인했고 DNS·HTTPS·배포 연결은 미확인이다.
...
| 크기 선택 | 4GB는 잠정 예산 기준, 2GB는 SSR·API·DB 부하 시험 통과 시 절약안 |
...
- 미확인: Java·Docker·adb는 현재 셸에서 명령을 찾지 못했다. 설치되지 않았다고 단정하지 않는다. WSL 배포판 기동·Linux 컨테이너·DB·Android 빌드도 아직 검증하지 않았다.
